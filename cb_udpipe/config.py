"""Načtení a ověření konfigurace modulu cb-udpipe.
...
závislosti (README-MODULES.md § 19). Validátor zvládá jen tu část JSON Schema,
kterou naše schémata používají, a na cokoli jiného **hlasitě upozorní** — nikdy
tiše neprojde. Nedostatečný validátor, který mlčí, je horší než žádný.

Tvar validátoru je převzatý z `cb_logger/config.py`; liší se konstanty, seznam
cest a kontrola rozsahu portů.
"""
...
#: Verze schématu, které tenhle kód rozumí. Když konfigurace nese jinou, start
#: selže — tiché načtení podle špatného předpokladu vyrobí čísla, která vypadají
#: správně (README-MODULES.md § 14).
SUPPORTED_CONFIG_VERSION = 1

#: Rozsah portů, který patří modulu (README-MODULES.md § 5). Modul si z něj bere
#: čísla vzestupně a nikdy nesahá do cizího. Kontroluje se proto, že jinak se
#: kolize pozná až tím, že se dva moduly poperou o totéž číslo — a to bývá za
#: provozu, ne při startu.
PORT_RANGE = (42200, 42299)

#: Klíče, jejichž hodnota je cesta. Vyjmenované schválně: hádání podle jména
#: („končí na _dir, tak je to cesta") by se rozešlo s obsahem, jakmile přibude
#: klíč, který se tak jmenuje a cesta není.
PATH_KEYS = (
    ("runtime", "pid_file"),
    ("runtime", "port_file"),
...
class ConfigError(Exception):
    ...
    návratový kód 2 (špatné argumenty nebo neplatná konfigurace,
...
def load(path: str | Path | None = None) -> dict[str, Any]:
    ...
    funkcím po jedné (README-MODULES.md § 3). Objekt, ze kterého si funkce sama
    tahá, co potřebuje, schová závislost před čtenářem signatury.
...
def _check_ports(config: dict[str, Any]) -> list[str]:
    """Ověří, že všechny porty leží v rozsahu modulu.

    Proč zvlášť a ne ve schématu: schéma umí `minimum` a `maximum`, ale nemá
    jak povolit nulu jako výjimku („přidělí systém") a zároveň zakázat
    1–42199. Napsat to jako dvě větve výčtem by znamenalo tři čísla na třech
    místech schématu; tady je to jedna podmínka s jednou hláškou.

    Vstup:
        config: surová konfigurace po ověření typů.

    Výstup:
        Seznam českých popisů chyb; prázdný, když jsou porty v pořádku.
    ...
    chyby: list[str] = []
    porty = (
        ("service.port", config.get("service", {}).get("port")),
        ("module.upstream.port",
         config.get("module", {}).get("upstream", {}).get("port")),
    )
    for jmeno, hodnota in porty:
        if not isinstance(hodnota, int) or isinstance(hodnota, bool):
            continue                    # špatný typ už ohlásilo schéma
        if hodnota == 0:
            continue                    # „přidělí systém" — používají testy
        if not od <= hodnota <= do:
            chyby.append(
                f"{jmeno}: {hodnota} je mimo rozsah modulu {od}–{do}. "
...
def _validate(hodnota: Any, schema: dict[str, Any], *, kde: str) -> list[str]:
    ...
    if isinstance(hodnota, list) and "items" in schema:
        for i, polozka in enumerate(hodnota):
            chyby += _validate(polozka, schema["items"], kde=f"{kde}[{i}]")
...
def _validate_object(
    ...
    if schema.get("additionalProperties") is False:
        for klic in sorted(set(hodnota) - set(vlastnosti)):
...
def _fingerprint(config: dict[str, Any]) -> str:
    ...
    dvě čísla nesrovnatelná (README-MODULES.md § 11). Verze v souboru se mění
    zřídka, kdežto hodnoty často — otisk pozná i změnu, u které nikdo číslo
    nezvýšil.
